...
(function () {
  ...
  function AlgoliaIntegration() {
    ...
    var CLIENT = algoliasearch(ALGOLIA_ID, ALGOLIA_KEY);
    var INDEX = CLIENT.initIndex(ALGOLIA_INDEX);
    // var CIENT_SUBDOMAIN = "yanpix-annamez";
    var CIENT_SUBDOMAIN = "yanpix-annamez";

    var custom_variables = [];
    var j = 0;
    var urlRiderect = 'https://'+CIENT_SUBDOMAIN+'.myanswers.io/redirecting?';
...
// Get stop words lists
    ...
    function getGroup(event) {
      var authorization = "b2xnYTdkZWluZWtvKzFAZ21haWwuY29tOjFjODliNjc5ZTFhZWY3YzlmMzNlYjE4ZWM2NzI2NTRh";
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
          ...
          var xhr2 = new XMLHttpRequest(),
            method = "GET",
            url = 'https://'+CIENT_SUBDOMAIN+'.myanswers.io/group-list-algolia-test.json';
              ...
              for (var i = 0; i < content.hits.length; i++) {
                ...
                custom_variables[j + 1] = {
                  ...
                  value: algoliaResults[i]._highlightResult.answer.value.toString().replace(/<(.*?)em(.*?)>/gm, '')
                    .split(' ').slice(0, 75).join(' ') + '. See more:' + urlRiderect + 'faq=' + algoliaResults[i].objectID
                };
                j = j + 2;
              }
              j = 0;
